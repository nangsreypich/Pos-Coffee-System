...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['process_order'])) {
    ...
    $cus_id = uniqid(); // Unique customer ID
    $invoice_id = uniqid(); // Unique invoice ID
    $payment_type = $_POST['payment_type'];
    $total_payment = $_POST['total_payment'];
    $change = $_POST['change'];
    $sale_date = date('Y-m-d'); // Current date
    $table_id = $_POST['table_id']; // Get the selected table ID
    ...
    try {
        ...
        foreach ($orderDetails as $orderItem) {
            ...
            $stmt->execute([$table_id, $invoice_id, $cus_id, $orderItem['drink_id'], $orderItem['price'], $orderItem['qty'], $total, $total_payment, $change, $payment_type, $sale_date]);
        }
